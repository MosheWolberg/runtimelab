...
namespace TypeLibUtilities.Event
{
    ...
    {
        private static readonly CustomAttributeBuilder HiddenCABuilder = CustomAttributeHelper.GetBuilderFor<TypeLibTypeAttribute>(TypeLibTypeFlags.FHidden);
        private static readonly CustomAttributeBuilder NoClassItfCABuilder = CustomAttributeHelper.GetBuilderFor<ClassInterfaceAttribute>(ClassInterfaceType.None);
        ...
        public Type Perform()
        {
            ...
            TypeBuilder OutputTypeBuilder = DefineUniqueType(
                                                             strFullName,
            ...
            SetHiddenAttribute(OutputTypeBuilder);

            // Set the class interface to none.
            SetClassInterfaceTypeToNone(OutputTypeBuilder);

            // Retrieve the property methods on the input interface and give them a dummy implementation.
            MethodInfo[] pMethods = GetPropertyMethods(m_InputType);
        ...
        private static void SetClassInterfaceTypeToNone(TypeBuilder tb)
        {
            // Set the class interface type to none.
            tb.SetCustomAttribute(NoClassItfCABuilder);
        }

        private static void SetHiddenAttribute(TypeBuilder tb)
        {
            tb.SetCustomAttribute(HiddenCABuilder);
        }

        private static TypeBuilder DefineUniqueType(string strInitFullName, TypeAttributes attrs, Type BaseType, Type[] aInterfaceTypes, ModuleBuilder mb)
        {
            int verId = 2;
            string strFullName = strInitFullName;

            // Find the first unique name for the type.
            while (mb.GetType(strFullName) != null)
            {
                strFullName = $"{strInitFullName}_{verId}";
                verId++;
            }
        ...
        private static MethodInfo[] GetPropertyMethods(Type type)
        {
