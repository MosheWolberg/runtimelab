...
namespace TypeLibUtilities
{
    ...
    /// <summary>
    /// External or local type
    /// </summary>
    internal enum ConvScope
    {
        /// <summary>
        /// The managed type is in the type library being converting
        /// </summary>
        Local,

        /// <summary>
        /// The managed type is already available in a different DLL
        /// </summary>
        External
    }

    /// <summary>
    /// Represents ITypeInfo => managed type conversion
    ///
    /// All ITypeInfo instances will have corresponding IConvBase instances, either local/external.
    ///
    ...
    /// 
    /// In order to abstract away the difference between local types & external types, use the following approach:
    /// Use an interface to derive from IConvBase, and have two different classes represent local/external type.
    /// We only need to access using this interface instead of using the class directly, thus
    ...
    /// 
    /// The derived classes (ConvInterfaceLocal, ConvInterfaceExternal) represent
    /// actual conversions (ConvCoClassLocal/External, ConvStructLocal/External,
    /// ConvInterfaceLocal/External, etc.). IConvBase is used for references and definitions.
    /// TypeBuilder is derived from Type and can be used to generate references to the type
    /// being constructed. During the conversion process, there can be an ITypeInfo encountered
    /// for a given type that hasn't been converted yet. This is typically when the type is
    /// used as a parameter or field type definition.
    ...
    /// </summary>
    internal interface IConvBase
    {
        /// <summary>
        /// The TypeInfo. The reason named "Ref" is because this property returns an existing TypeInfo instead of a new one
        /// </summary>
        TypeInfo RefTypeInfo { get; }

        /// <summary>
        /// The always non-aliased TypeInfo
        /// </summary>
        TypeInfo RefNonAliasedTypeInfo { get; }
        ...
        /// </summary>
        ConvType ConvType { get; }

        /// <summary>
        /// Scope => Local / External
        /// </summary>
        ConvScope ConvScope { get; }
